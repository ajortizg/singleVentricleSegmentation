...
if __name__ == "__main__":
    ...
    train_ds = SingleVentricleDataset(config, DatasetMode.FULL, load_flow=False, data_transforms=transf)
    ...
    for ds in [train_ds]:
        for (pname, data, m0, mk, init_ts, final_ts, _, _) in ds:
            ...
            # plots.save_img_masks_slices(u0, [m0, erode(m0)], save_dir, 'm0', 0.5, alphas=[0.2, 1.0], colors=[[1, 0.75, 0], [0, 1, 0]])
            # plots.save_img_masks_slices(uk, [mk, erode(mk)], save_dir, 'mk', 0.5, alphas=[0.2, 1.0], colors=[[0, 0.75, 1], [1, 0, 0]])
