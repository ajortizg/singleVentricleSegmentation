...
class RandomRotateGPU:
    def __init__(self, 
                 config,
                 p=0.5, range_x: tuple = (0, 0),
                 range_y: tuple = (0, 0),
                 range_z: tuple = (0, 0),
                 total: int = None, 
                 interp,
                 boundary,
                 clip_interval: tuple = (0.0, 1.0)):
        self.config = config
        self.p = p
        self.range_x = range_x
        self.range_y = range_y
    ...
    def __call__(self, img4d: np.array, ms: np.array, md: np.array, ff: np.array, bf: np.array):
        ...
        if np.random.rand() < self.p:
            ...
            ff_rot[t, :, :, :, c] = rotationOp.forwardMatrixField(ff, R, offset)
            bf_rot[t, :, :, :, c] = rotationOp.forwardMatrixField(bf, R, offset)
