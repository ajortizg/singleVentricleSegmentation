...
template <typename T>
__global__ void cuda_nabla3d_cd_forwardVectorField_kernel(
...
template <typename T>
__global__ void cuda_divergence3d_cd_backwardVectorField_kernel(
  ...
  const torch::Tensor &b, const MeshInfo3D &meshInfo)
{
  ...
  AT_DISPATCH_FLOATING_TYPES(b.type(), "nabla3d_cd_forwardVectorField", ([&]{
    cuda_nabla3d_cd_forwardVectorField_kernel<scalar_t><<<numBlocks, blockSize>>>(
  ...
  const torch::Tensor &p, const MeshInfo3D &meshInfo)
{
  ...
  AT_DISPATCH_FLOATING_TYPES(p.type(), "divergence3d_cd_backwardVectorField", ([&]{
    cuda_divergence3d_cd_backwardVectorField_kernel<scalar_t><<<numBlocks, blockSize>>>(
