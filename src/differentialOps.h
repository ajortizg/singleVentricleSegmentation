...
torch::Tensor cuda_nabla3d_cd_forwardVectorField( const torch::Tensor &b, const MeshInfo3D &meshInfo);
torch::Tensor cuda_divergence3d_cd_backwardVectorField( const torch::Tensor &b, const MeshInfo3D &meshInfo);
...
public:
  ...
  torch::Tensor forwardVectorField(const torch::Tensor &b) const {
    CHECK_INPUT(b);
    return cuda_nabla3d_cd_forwardVectorField(b,_meshInfo);
  }
  torch::Tensor backwardVectorField(const torch::Tensor &b) const{
    CHECK_INPUT(b);
    return cuda_divergence3d_cd_backwardVectorField(b,_meshInfo);
  }
};
