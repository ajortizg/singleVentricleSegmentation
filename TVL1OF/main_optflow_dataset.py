...
class TVL1OpticalFlow3D_Dataset:
    ...
    def compute_optical_flow(self, ds: SingleVentricleDataset, idx: int, mode: OpticalFlowMode, save_dir: str, device: str, step: int, logger):
        ...
        for i in range(len(idxs) - 1):
            ...
            algPatient = TVL1OpticalFlow3D(save_dir_timestep, self.config)
...
if __name__ == "__main__":
    ...
    saveDir = plots.createSaveDirectory(config.get('DATA', 'OUTPUT_PATH'), 'TVL1OF3D')
    ...
    print("mode =", modeStr)
    alg.compute(mode_str=modeStr,device=DEVICE)
