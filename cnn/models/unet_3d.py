...
#         act = config.get('PARAMETERS', 'ACTIVATION')
#         slope = config.getfloat('PARAMETERS', 'ACTIVATION_SLOPE')
#         lipschitz = config.getboolean('PARAMETERS', 'LIPSCHITZ_REGULARIZATION')
...
#         super().__init__()
#         self.num_layers = num_layers
#         layers = [DoubleConv3d(input_channels, features_start, kernel_size, padding, act, slope, lipschitz)]

#         feats = features_start
#         for _ in range(num_layers - 1):
#             layers.append(Down3d(feats, feats * 2, kernel_size, padding, act, slope, lipschitz))
#             feats *= 2

#         for _ in range(num_layers - 1):
#             layers.append(Up3d(feats, feats // 2, trilinear, kernel_size, padding, act, slope, lipschitz))
#             feats //= 2

#         out_conv = nn.Conv3d(feats, num_classes, kernel_size=1)
#         if lipschitz:
#             out_conv = parametrizations.spectral_norm(out_conv)
#         layers.append(out_conv)
...
# class DoubleConv3d(nn.Module):
#     def __init__(self, in_ch: int, out_ch: int, kernel_size=(3, 3, 3), padding=1, act='relu', slope=0.2, lipschitz=False):
#         super().__init__()
#         conv1 = nn.Conv3d(in_ch, out_ch, kernel_size=kernel_size, padding=padding)
#         conv2 = nn.Conv3d(out_ch, out_ch, kernel_size=kernel_size, padding=padding)
#         if lipschitz:
#             self.net = nn.Sequential(parametrizations.spectral_norm(conv1),
#                                      self.activation(act, slope),
#                                      parametrizations.spectral_norm(conv2),
#                                      self.activation(act, slope))
#         else:
#             self.net = nn.Sequential(conv1,
#                                      nn.InstanceNorm3d(out_ch),
...
#         self.net = nn.Sequential(
#             nn.MaxPool3d(kernel_size=2, stride=2),
#             DoubleConv3d(in_ch, out_ch, kernel_size, padding, act, slope, lipschitz)
#         )
...
#                  padding=1, act='relu', slope=0.2, lipschitz=False):
#         super().__init__()
#         self.upsample = None
#         if trilinear:
#             self.upsample = nn.Sequential(
#                 nn.Upsample(scale_factor=2, mode="trilinear", align_corners=True),
#                 parametrizations.spectral_norm(nn.Conv3d(in_ch, in_ch // 2, kernel_size=1)) if lipschitz else nn.Conv3d(in_ch, in_ch // 2, kernel_size=1))
#         else:
#             if lipschitz:
#                 self.upsample = parametrizations.spectral_norm(nn.ConvTranspose3d(in_ch, in_ch // 2, kernel_size=2, stride=2))
#             else:
#                 self.upsample = nn.ConvTranspose3d(in_ch, in_ch // 2, kernel_size=2, stride=2)

#         self.conv = DoubleConv3d(in_ch, out_ch, kernel_size=kernel_size, padding=padding, act=act, slope=slope, lipschitz=lipschitz)
